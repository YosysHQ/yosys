...
void logv_header(RTLIL::Design *design, const char *format, va_list ap)
{
	...
	logv(LogSeverity::INFO, format, ap);
	log_flush();
                                     ...
                                     const char *format, va_list ap)
{
	...
	else
	{
		...
		else
		{
			...
			log(LogSeverity::WARNING, "%s%s", prefix, message.c_str());
			log_flush();
                      ...
                      const char *format, ...)
{
	...
	logv(LogSeverity::INFO, fmt.c_str(), ap);
	va_end(ap);
}
                                   ...
                                   const char *format, va_list ap)
{
	...
	log(LogSeverity::ERROR, "%s%s", prefix, log_last_error.c_str());
	log_flush();
...
void log_cmd_error(const char *format, ...)
{
	...
	if (log_cmd_error_throw) {
		...
		log(LogSeverity::ERROR, "ERROR: %s", log_last_error.c_str());
		log_flush();
