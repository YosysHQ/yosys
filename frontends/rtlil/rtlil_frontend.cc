...
struct RTLILFrontendWorker {
	...
	RTLIL::SigSpec parse_sigspec()
	{
		...
		std::optional<RTLIL::IdString> id = try_parse_id();
		if (id.has_value()) {
			if (!current_module->count_id(*id))
				error("Wire `%s' not found.", *id);
			sig = RTLIL::SigSpec(current_module->wire(*id));
		} else {
	...
	RTLILFrontendWorker(std::istream *f, RTLIL::Design *design)
			: f(f), design(design) {}
...
struct RTLILFrontend : public Frontend {
	...
	{
		RTLILFrontendWorker worker(f, design);
		...
		extra_args(worker.f, filename, args, argidx);
