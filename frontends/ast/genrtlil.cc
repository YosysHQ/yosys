		...
		case AST_CASE:
			{
				...
				sw->signal_src = src_ref(ast->children[0].get());
				...
				for (auto& child : ast->children)
				{
					...
					current_case->compare_src = src_ref(child.get());
...
std::vector<RTLIL::Binding *> AstNode::genBindings() const
{
	...
	for (int i = first_tgt_inst; i < num_ids; ++i) {
		...
		for (int j = num_ids; j < GetSize(children); ++j) {
			...
			ret.push_back(new AST::Binding(current_module->design->twines.add(std::string(tgt_type)),
			                               current_module->design->twines.add(std::string(tgt_child.str)),
			                               cell_child));
		}
	}
