...
.. literalinclude:: /cmd/techlibs/synth_ice40.rst
   :language: yoscrypt
   :start-after: begin:
   :end-before: flatten:
   :dedent:
...
To handle these, let us now introduce the next command: :doc:`/cmd/passes/proc`.
...
.. note::
   ...
   :doc:`/cmd/passes/clean` can also be called with two semicolons after any command,
...
you can do so with :doc:`/cmd/passes/show`.  Note that the `show` command only
works with a single module, so you may need to call it with :yoscrypt:`show
fifo`. :ref:`show_intro` section in :doc:`/getting_started/scripting_intro` has
more on how to use `show`.
...
:doc:`/cmd/passes/fsm`.  Both `opt` and `fsm` are macro commands which are
explored in more detail in :doc:`/using_yosys/synthesis/opt` and
...
First up is :doc:`/cmd/passes/wreduce`.  If we run this we get the following:
...
The next two (new) commands are :doc:`/cmd/passes/peepopt` and
:doc:`/cmd/passes/share`. Neither of these affect our design, and they're
explored in more detail in :doc:`/using_yosys/synthesis/opt`, so let's skip over
them.  :yoscrypt:`techmap -map +/cmp2lut.v -D LUT_WIDTH=4` optimizes certain
comparison operators by converting them to LUTs instead.  The usage of `techmap`
is explored more in :doc:`/using_yosys/synthesis/techmap_synth`.

Our next command to run is
:doc:`/cmd/passes/memory_dff`.
...
The other new command in this part is :doc:`/cmd/passes/memory`.  `memory` is
...
- :doc:`/cmd/passes/autoname`,
- :doc:`/cmd/passes/stat`, and
- :doc:`/cmd/passes/blackbox`.
...
- :doc:`/cmd/backends/write_blif`,
- :doc:`/cmd/backends/write_edif`, and
- :doc:`/cmd/backends/write_json`.
...
.. seealso:: :doc:`/cmd/techlibs/synth_ice40`
