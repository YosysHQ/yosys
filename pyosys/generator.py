...
class PyosysWrapperGenerator(object):
    ...
    def process_field(self, metadata: PyosysClass, field: Field):
        ...
        if has_containers:
            # always reference with lifetime managed by c++
            # never copy containers.
            def_args.append("py::return_value_policy::reference")
        else:
            # automatic reference: managed by c++ lifetime if pointer, copied to
            # python if rvalue, moved to python if lvalue
            def_args.append("py::return_value_policy::automatic_reference")
        print(
