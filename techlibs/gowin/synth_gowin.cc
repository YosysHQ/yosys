...
struct SynthGowinPass : public ScriptPass
{
	...
	void help() override
	{
		...
		log("\n");
		log("    -dsp\n");
		log("        infer DSP multipliers (EXPERIMENTAL)\n");
		log("\n");
	...
	bool retime, nobram, nolutram, flatten, nodffe, strict_gw5a_dffs, nowidelut, abc9, noiopads, noalu, no_rw_check, setundef, dsp;
	...
	void script() override
	{
		...
		if (check_label("coarse"))
		{
			...
			if (help_mode) {
				run("techmap -map +/mul2dsp.v [...]", "(if -dsp and if -family gw1n or gw2a)");
				run("techmap -map +/gowin/dsp_map.v", "(if -dsp and if -family gw1n or gw2a)");
			} else if (dsp && (family == "gw1n" || family == "gw2a")) {
