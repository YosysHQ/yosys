...
void invert_gp_dff(Cell *cell, bool invert_input)
{
	...
	if(cell_type_latch)
		cell->type = cell->module->design->twines.add(std::string{stringf("\\GP_DLATCH%s%s%s", cell_type_s ? "S" : "", cell_type_r ? "R" : "", cell_type_i ? "I" : "")});
	else
		cell->type = cell->module->design->twines.add(std::string{stringf("\\GP_DFF%s%s%s", cell_type_s ? "S" : "", cell_type_r ? "R" : "", cell_type_i ? "I" : "")});
