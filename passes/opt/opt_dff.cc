...
struct OptDffWorker
{
	...
	SigSpec create_not(SigSpec a, bool is_fine, Cell *cell) {
		if (is_fine)
			return module->NotGate(NEW_ID2_SUFFIX("not"), a, cell->get_src_attribute());
		else
			return module->Not(NEW_ID2_SUFFIX("not"), a, false, cell->get_src_attribute());
	}

	SigSpec create_and(SigSpec a, SigSpec b, bool is_fine, Cell *cell) {
		if (is_fine)
			return module->AndGate(NEW_ID2_SUFFIX("and"), a, b, cell->get_src_attribute());
		else
			return module->And(NEW_ID2_SUFFIX("and"), a, b, false, cell->get_src_attribute());
	}

	void create_mux_to_output(SigSpec a, SigSpec b, SigSpec sel, SigSpec y, bool pol, bool is_fine, Cell *cell) {
		if (is_fine) {
			if (pol)
				module->addMuxGate(NEW_ID2_SUFFIX("mux"), a, b, sel, y, cell->get_src_attribute());
			else
	...
	ctrl_t combine_resets(const ctrls_t &ctrls, bool make_gates, Module *module, Cell *cell)
	{
		...
		RTLIL::Cell *c = final_pol
			? module->addReduceOr(NEW_ID2_SUFFIX("comb_rst_reduce_or"), or_input, y, false, cell->get_src_attribute()) // SILIMATE: Improve the naming
			: module->addReduceAnd(NEW_ID2_SUFFIX("comb_rst_reduce_and"), or_input, y, false, cell->get_src_attribute()); // SILIMATE: Improve the naming
		maybe_simplemap(c, make_gates);
		...
		for (int i = 0; i < ff.width; i++) {
			if (is_always_active(ff.sig_clr[i], ff.pol_clr)) {
				...
				if (!ff.pol_clr)
					...
					module->addNotGate(NEW_ID2_SUFFIX("not"), ff.sig_clr[i], ff.sig_q[i], cell->get_src_attribute());
				else
	...
	bool optimize_aload(FfData &ff, Cell *cell, bool &changed)
	{
		...
		if (is_active(ff.sig_aload, ff.pol_aload)) {
			...
			if (ff.has_sr) {
				...
				if (ff.is_fine) {
					tmp = ff.pol_set
						? module->MuxGate(NEW_ID2_SUFFIX("mux"), ff.sig_ad, State::S1, ff.sig_set, cell->get_src_attribute())
						: module->MuxGate(NEW_ID2_SUFFIX("mux"), State::S1, ff.sig_ad, ff.sig_set, cell->get_src_attribute());

					if (ff.pol_clr)
						module->addMuxGate(NEW_ID2_SUFFIX("mux"), tmp, State::S0, ff.sig_clr, ff.sig_q, cell->get_src_attribute());
					else
						module->addMuxGate(NEW_ID2_SUFFIX("mux"), State::S0, tmp, ff.sig_clr, ff.sig_q, cell->get_src_attribute());
				} else {
					tmp = ff.pol_set
						? module->Or(NEW_ID2_SUFFIX("or"), ff.sig_ad, ff.sig_set, false, cell->get_src_attribute())
						: module->Or(NEW_ID2_SUFFIX("or"), ff.sig_ad, module->Not(NEW_ID2_SUFFIX("not"), ff.sig_set, false, cell->get_src_attribute()), false, cell->get_src_attribute());

					if (ff.pol_clr)
						module->addAnd(NEW_ID2_SUFFIX("and"), tmp, module->Not(NEW_ID2_SUFFIX("not"), ff.sig_clr, false, cell->get_src_attribute()), ff.sig_q, false, cell->get_src_attribute());
					else
						module->addAnd(NEW_ID2_SUFFIX("and"), tmp, ff.sig_clr, ff.sig_q, false, cell->get_src_attribute());
				}
			} else if (ff.has_arst) {
