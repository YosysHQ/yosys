...
SigSpec OptDffWorker::create_not(SigSpec a, bool is_fine) {
	if (is_fine)
...
SigSpec OptDffWorker::create_and(SigSpec a, SigSpec b, bool is_fine) {
	if (is_fine)
...
void OptDffWorker::create_mux_to_output(SigSpec a, SigSpec b, SigSpec sel, SigSpec y, bool pol, bool is_fine) {
	if (is_fine) {
		if (pol)
...
void OptDffWorker::maybe_simplemap(Cell *c, bool make_gates) {
	if (make_gates) {
		simplemap(module, c);
		module->remove(c);
	}
...
patterns_t OptDffWorker::find_muxtree_feedback_patterns(RTLIL::SigBit d, RTLIL::SigBit q, pattern_t path)
{
...
ctrl_t OptDffWorker::make_patterns_logic(const patterns_t &patterns, const ctrls_t &ctrls, bool make_gates)
{
...
ctrl_t OptDffWorker::combine_resets(const ctrls_t &ctrls, bool make_gates)
{
	if (GetSize(ctrls) == 1)
		return *ctrls.begin();
...
bool OptDffWorker::signal_all_same(const SigSpec &sig) {
...
bool OptDffWorker::optimize_sr(FfData &ff, Cell *cell, bool &changed)
{
...
bool OptDffWorker::optimize_aload(FfData &ff, Cell *cell, bool &changed)
{
...
bool OptDffWorker::optimize_arst(FfData &ff, Cell *cell, bool &changed)
{
...
void OptDffWorker::optimize_srst(FfData &ff, Cell *cell, bool &changed)
{
...
void OptDffWorker::optimize_ce(FfData &ff, Cell *cell, bool &changed)
{
...
void OptDffWorker::optimize_const_clk(FfData &ff, Cell *cell, bool &changed)
{
...
void OptDffWorker::optimize_d_equals_q(FfData &ff, Cell *cell, bool &changed)
{
...
bool OptDffWorker::try_merge_srst(FfData &ff, Cell *cell, bool &changed)
{
...
bool OptDffWorker::try_merge_ce(FfData &ff, Cell *cell, bool &changed)
{
