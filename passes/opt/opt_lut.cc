...
struct OptLutWorker
{
	...
	void show_stats_by_arity()
	{
		...
		{
			log("  with %-12s (#%d) %4d\n", module->twines().str(dlogic[i].cell_type), i, dlogic_counts[i]);
		}
	}
			...
			if (cell->type == ID($lut))
			{
				...
				for (auto lut_dlogic : lut_all_dlogics)
				{
					...
					for (auto &dlogic_conn : dlogic_map)
					{
						...
						{
							log_debug("  LUT has illegal connection to %s cell %s.%s.\n", lut_dlogic.second->type, module, lut_dlogic.second);
							log_debug("    LUT input A[%d] (wire %s) not connected to %s port %s (wire %s).\n", dlogic_conn.first, log_signal(lut_input[dlogic_conn.first]), lut_dlogic.second->type, module->twines().str(dlogic_conn.second).c_str(), log_signal(lut_dlogic.second->getPort(dlogic_conn.second)));
							legal = false;
							break;
						}
					}
