...
// Check that all the connections and parameters on the cell match those defined
// in its type.
...
bool expand_module(RTLIL::Design *design, RTLIL::Module *module, bool flag_check, bool flag_simcheck, std::vector<std::string> &libdirs)
{
	...
	for (auto cell : module->cells())
	{
		...
		if (!mod)
		{
			...
			// If we still don't have a module, treat the cell as a
			// black box and skip it.
			if (!mod)
				continue;

			// Otherwise, we either loaded or derived something
			did_something = true;
		}
