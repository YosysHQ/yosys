...
struct TribufWorker {
	...
	void run()
	{
				...
				for (auto it : current_tribufs) {
					...
					if (driving_cells.at(it).size() > 1) {
						if (config.merge_mode) {
							merge(it, driving_cells, tribuf_signals);
		...
		if (config.merge_mode || config.logic_mode || config.formal_mode) {
			while (!tribuf_signals.empty()) {
				auto it = *tribuf_signals.begin();
				auto const driving_cells = get_driving_cells();
	...
	void merge(RTLIL::SigBit sig, dict<SigBit, pool<Cell *>> const &driving_cells, pool<SigBit> &tribuf_signals)
	{
		// sigmap.set(module);
