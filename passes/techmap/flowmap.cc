...
struct GraphStyle {
	...
	GraphStyle(string label = "", string color = "black", string fillcolor = "") : label(label), color(color), fillcolor(fillcolor) {}
};
...
static void dump_dot_graph(
  string filename, pool<RTLIL::SigBit> nodes, dict<RTLIL::SigBit, pool<RTLIL::SigBit>> edges, pool<RTLIL::SigBit> inputs, pool<RTLIL::SigBit> outputs,
  std::function<GraphStyle(RTLIL::SigBit)> node_style = [](RTLIL::SigBit) { return GraphStyle{}; },
  std::function<GraphStyle(RTLIL::SigBit, RTLIL::SigBit)> edge_style = [](RTLIL::SigBit, RTLIL::SigBit) { return GraphStyle{}; }, string name = "")
{
	...
	for (auto node : nodes) {
		...
		fprintf(f, "  n%d [ shape=%s, fontname=\"Monospace\", label=\"%s\", color=\"%s\", fillcolor=\"%s\", style=\"%s\" ];\n", ids[node],
			shape.c_str(), dot_escape(prop.label.c_str()).c_str(), prop.color.c_str(), prop.fillcolor.c_str(), style.c_str());
	}
		...
		for (auto sink : edge.second) {
			if (nodes[source] && nodes[sink]) {
				...
				fprintf(f, "  n%d -> n%d [ label=\"%s\", color=\"%s\", fillcolor=\"%s\" ];\n", ids[source], ids[sink],
					dot_escape(prop.label.c_str()).c_str(), prop.color.c_str(), prop.fillcolor.c_str());
			}
		}
	}
...
struct FlowGraph {
	...
	struct NodePrime {
		...
		NodePrime(RTLIL::SigBit node, bool is_bottom) : node(node), is_bottom(is_bottom) {}

		bool operator==(const NodePrime &other) const { return node == other.node && is_bottom == other.is_bottom; }
		bool operator!=(const NodePrime &other) const { return !(*this == other); }
		unsigned int hash() const { return hash_ops<pair<RTLIL::SigBit, int>>::hash({node, is_bottom}); }

		static NodePrime top(RTLIL::SigBit node) { return NodePrime(node, /*is_bottom=*/false); }

		static NodePrime bottom(RTLIL::SigBit node) { return NodePrime(node, /*is_bottom=*/true); }
...
struct FlowmapWorker {
	...
	void dump_dot_graph(string filename, GraphMode mode, pool<RTLIL::SigBit> subgraph_nodes = {},
			    dict<RTLIL::SigBit, pool<RTLIL::SigBit>> subgraph_edges = {}, dict<RTLIL::SigBit, pool<RTLIL::SigBit>> collapsed = {},
			    pair<pool<RTLIL::SigBit>, pool<RTLIL::SigBit>> cut = {})
	{
		...
		auto edge_style = [&](RTLIL::SigBit, RTLIL::SigBit) { return GraphStyle{}; };
	...
	void compute_lut_distances(dict<RTLIL::SigBit, int> &lut_distances, bool forward, pool<RTLIL::SigBit> initial = {},
				   pool<RTLIL::SigBit> *changed = nullptr)
	{
	...
	void compute_lut_critical_outputs(dict<RTLIL::SigBit, pool<RTLIL::SigBit>> &lut_critical_outputs, pool<RTLIL::SigBit> worklist = {})
	{
		if (worklist.empty())
			worklist = lut_nodes;
					...
					for (auto lut_succ : lut_edges_fw[lut]) {
						if (lut_nodes[lut_succ] && lut_depths[lut_succ] == lut_depths[lut] + 1) {
							if (lut_critical_outputs.count(lut_succ))
								lut_critical_outputs[lut].insert(lut_critical_outputs[lut_succ].begin(),
	...
	void update_lut_critical_outputs(dict<RTLIL::SigBit, pool<RTLIL::SigBit>> &lut_critical_outputs, pool<RTLIL::SigBit> worklist = {})
	{
		if (!worklist.empty()) {
	...
	bool relax_depth_for_bound(bool first, int depth_bound, dict<RTLIL::SigBit, pool<RTLIL::SigBit>> &lut_critical_outputs)
	{
		...
		for (int break_num = 1;; break_num++) {
			...
			log("  Breaking LUT %s to %s LUT %s (potential %d).\n", log_signal(breaking_lut),
			    lut_nodes[breaking_gate] ? "reuse" : "extract", log_signal(breaking_gate), best_potential);
			...
			while (!worklist.empty()) {
				...
				// We are invalidating the entire output cone of the gate IR node, not just of the LUT IR node. This is done to also
				// invalidate all LUTs that could contain one of the indirectly affected nodes as a *part* of them, as they may not be
				// in the output cone of any of the LUT IR nodes, e.g. if we have a LUT IR node A and node B as predecessors of node
				// C, where node B includes all gates from node A.
		...
		for (auto node : lut_nodes) {
			if (node_origins.count(node)) {
				...
				if (origin.cell->getPort(origin.port).size() == 1)
					...
					log("Packing %s.%s.%s [%d] (%s).\n", log_id(module), log_id(origin.cell), origin.port.c_str(), origin.offset,
					    log_signal(node));
			} else {
			...
			for (auto gate_node : lut_gates[node]) {
				...
				if (gate_origin.cell->getPort(gate_origin.port).size() == 1)
					log("  Packing %s.%s.%s (%s).\n", log_id(module), log_id(gate_origin.cell), gate_origin.port.c_str(),
					    log_signal(gate_node));
				else
					log("  Packing %s.%s.%s [%d] (%s).\n", log_id(module), log_id(gate_origin.cell), gate_origin.port.c_str(),
					    gate_origin.offset, log_signal(gate_node));
			}
	...
	FlowmapWorker(int order, int minlut, pool<IdString> cell_types, int r_alpha, int r_beta, int r_gamma, bool relax, int optarea, bool debug,
		      bool debug_relax, RTLIL::Module *module)
	    : order(order), r_alpha(r_alpha), r_beta(r_beta), r_gamma(r_gamma), debug(debug), debug_relax(debug_relax), module(module),
	      sigmap(module), index(module)
	{
		log("Labeling cells.\n");
