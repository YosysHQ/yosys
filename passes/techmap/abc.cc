...
std::string AbcModuleState::remap_name(RTLIL::IdString abc_name, RTLIL::Wire **orig_wire)
{
	...
	if (abc_sname.compare(0, 5, "ys__n") == 0)
	{
		...
		if (std::isdigit(abc_sname.at(0)))
		{
			...
			if (sid < GetSize(run_abc.signal_list))
			{
				...
				if (bit.wire != nullptr)
				{
					std::string s = stringf("$abc$%d$%d$%s", map_autoidx, state_index, bit.wire->name.c_str()+1);
					if (bit.wire->width != 1)
	...
	return stringf("$abc$%d$%d$%s", map_autoidx, state_index, abc_name.substr(1));
}
...
void AbcModuleState::handle_loops(AbcSigMap &assign_map, RTLIL::Module *module)
{
	...
	while (workpool.size() > 0)
	{
		...
		while (workpool.size() == 0)
		{
			...
			RTLIL::Wire *wire = module->addWire(stringf("$abcloop$%d$%d$%d", map_autoidx, state_index, loop_count++));
...
void AbcModuleState::extract(RTLIL::Design *design, RTLIL::Module *module)
{
	...
	for (auto w : mapped_mod->wires()) {
		...
		if (markgroups) wire->attributes[ID::abcgroup] = stringf("%d_%d", map_autoidx, state_index);
	...
	for (auto c : mapped_mod->cells())
	{
		if (builtin_lib)
		{
			...
			if (c->type == ID(NOT)) {
				...
				if (markgroups) cell->attributes[ID::abcgroup] = stringf("%d_%d", map_autoidx, state_index);
...
struct AbcPass : public Pass {
	...
	void help() override
	{
		...
		log("        this attribute is a unique string for each ABC process started. This\n");
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		struct AbcRunState {
			...
			AbcRunState(const AbcConfig &config, Yosys::RTLIL::Module *mod, std::shared_ptr<AbcSigMap> assign_map,
					std::shared_ptr<FfInitVals> initvals, int state_index, int map_autoidx)
					: mod(mod), module_state(config, *initvals, state_index, map_autoidx),
		...
		auto extract_runs = [&] {
			...
			while (next_state_index_to_process < GetSize(work_finished_by_index) &&
				...
				work->module_state.extract(design, work->mod);
		...
		for (auto mod : sorted_modules)
		{
			...
			if (!dff_mode || !clk_str.empty()) {
				std::unique_ptr<AbcRunState> run_state = std::make_unique<AbcRunState>(
						config, mod, std::move(assign_map), std::move(initvals), state_index++, autoidx_base);
			...
			for (auto &it : assigned_cells) {
				...
				std::unique_ptr<AbcRunState> run_state = std::make_unique<AbcRunState>(
						config, mod, assign_map, initvals, state_index, autoidx_base);
