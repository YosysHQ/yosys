	...
	struct alunode_t
	{
		...
		RTLIL::SigSpec get_gt(bool this_signed) {
			if (this_signed) {
				if (GetSize(cached_sgt) == 0) {
					...
					cached_sgt = alu_cell->module->Not(NEW_ID, Or, false, alu_cell->src_id());
				}
				...
				if (GetSize(cached_gt) == 0) {
					...
					cached_gt = alu_cell->module->Not(NEW_ID, Or, false, alu_cell->src_id());
				}
		...
		RTLIL::SigSpec get_eq() {
			if (GetSize(cached_eq) == 0)
				cached_eq = alu_cell->module->ReduceAnd(NEW_ID, alu_cell->getPort(ID::X), false, alu_cell->src_id());
			return cached_eq;
		}

		RTLIL::SigSpec get_ne() {
			if (GetSize(cached_ne) == 0)
				cached_ne = alu_cell->module->Not(NEW_ID, get_eq(), false, alu_cell->src_id());
			return cached_ne;
		}
		...
		RTLIL::SigSpec get_cf() {
			if (GetSize(cached_cf) == 0) {
				...
				cached_cf = alu_cell->module->Not(NEW_ID, cached_cf[GetSize(cached_cf)-1], false, alu_cell->src_id());
			}
			return cached_cf;
		}
