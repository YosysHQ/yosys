			...
			for (auto cell : module->selected_cells())
				for (auto port : cell->connections()) {
					...
					if (it2->second.size() != sig.size())
						log_error(
						  "The inversion parameter needs to be the same width as the port (%s.%s port %s parameter %s)",
						  log_id(module->name), log_id(cell->type), log_id(port.first), log_id(param_name));
					...
					for (int i = 0; i < sig.size(); i++)
						if (invmask[i] == State::S1) {
							...
							log("Inserting %s on %s.%s.%s[%d].\n", inv_celltype.c_str(), log_id(module),
							    log_id(cell->type), log_id(port.first), i);
