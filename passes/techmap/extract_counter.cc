...
int counter_tryextract(ModIndex &index, Cell *cell, CounterExtraction &extract, CounterExtractionSettings settings)
{
	...
	if (bi_port.is_fully_const() && bi_port.as_int() == 1 && ci_port.is_fully_const() && ci_port.as_int() == 1) {
		alu_is_subtract = true;
	} else if (bi_port.is_fully_const() && bi_port.as_int() == 0 && ci_port.is_fully_const() && ci_port.as_int() == 0) {
	...
	if (muxload->type == ID($mux)) {
		...
		if (sigmap(cemux->getPort(ID::B)) == muxy && sigmap(cemux->getPort(ID::A)) == sigmap(count_reg->getPort(ID::Q))) {
			extract.ce_inverted = false;
		} else if (sigmap(cemux->getPort(ID::A)) == muxy && sigmap(cemux->getPort(ID::B)) == sigmap(count_reg->getPort(ID::Q))) {
...
void counter_worker(ModIndex &index, Cell *cell, unsigned int &total_counters, pool<Cell *> &cells_to_remove,
		    pool<pair<Cell *, string>> &cells_to_rename, CounterExtractionSettings settings)
{
	SigMap &sigmap = index.sigmap;
	...
	if (port_wire->attributes.find(ID(COUNT_EXTRACT)) != port_wire->attributes.end()) {
		...
		if (sa.size() >= 1) {
			...
			log("  Signal %s declared at %s has COUNT_EXTRACT = %s\n", log_id(port_wire), count_reg_src.c_str(), extract_value.c_str());
	...
	if (reason != 0) {
		...
		if (force_extract) {
			log_error("Counter extraction is set to FORCE on register %s, but a counter could not be inferred (%s)\n", log_id(port_wire),
				  reasons[reason]);
		}
		return;
	}
	...
	log("  Found %d-bit (%s) %s counter %s (counting %s %d) for register %s, declared at %s\n", extract.width, reset_type.c_str(),
	    extract.count_is_up ? "up" : "down", countname.c_str(), extract.count_is_up ? "to" : "from", extract.count_value,
	    log_id(extract.rwire->name), count_reg_src.c_str());
