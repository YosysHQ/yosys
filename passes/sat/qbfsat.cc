...
void assume_miter_outputs(RTLIL::Module *module, bool assume_neg) {
	...
	if (assume_neg) {
		for (unsigned int i = 0; i < wires_to_assume.size(); ++i) {
			RTLIL::SigSpec n_wire = module->LogicNot(wires_to_assume[i]->name.str() + "__n__qbfsat", wires_to_assume[i], false, wires_to_assume[i]->src_id());
	...
	for (auto i = 0; wires_to_assume.size() > 1; ++i) {
		...
		for (auto j = 0; j + 1 < GetSize(wires_to_assume); j += 2) {
			...
			module->addLogicAnd("$_qbfsat_and_" + strstr.str(), wires_to_assume[j], wires_to_assume[j+1], and_wire, false, wires_to_assume[j]->src_id());
