			...
			for (auto cell : vector<Cell*>(module->selected_cells()))
			{
				if (cell->type.in(ID($print), ID($check)))
				{
					...
					if (trg_width == 0) {
						...
						module->addDff(NEW_ID, sig_trg, sig_en, sig_en_q, trg_polarity, cell->src_id());
						module->addDff(NEW_ID, sig_trg, sig_args, sig_args_q, trg_polarity, cell->src_id());
						...
						if (cell->type == ID($check)) {
							...
							module->addDff(NEW_ID, sig_trg, sig_a, sig_a_q, trg_polarity, cell->src_id());
